...
%outcomes are described as tuples of values of \emph{issues} (also 
%referred to as \emph{variables} or \emph{attributes}), say $X_1,\ldots, 
%X_p$, with each issue $X_i$ assuming values from some set $D_i$ -- its 
%\emph{domain}. 
	  ...
	  \begin{tikzpicture}[->,>=stealth',
	    ...
	    \node [main node,inner sep=0.5pt] (1){$X_2$}
	      child {node [main node,inner sep=0.5pt] (2) {$X_3$}
							...
							child {node [rectangle,draw] (5) at (0.5,0.9) {$0_3\!\!>\!\!1_3$} edge from parent[draw=none]}
	      };
	  \end{tikzpicture}
		\vspace{-0.2cm}
