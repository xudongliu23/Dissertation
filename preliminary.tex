...
language. To establish the correpondence, it suffices to select one
value in the domain of each attribute as \tit{True} and the other one
as \tit{False}.
...
We write variables $C$, $P$ and $S$ in propositional logic, 
corresponding to attributes
\tit{Capacity}, \tit{Price}, and \tit{Safety}.
We then set that the variables $C$, $P$ and $S$ being \tit{True} represent \tit{high} in
the attributes \tit{Capacity}, \tit{Price} and \tit{Safety}, repectively.
Immediately, the variables being \tit{False} means \tit{low} in the attributes.
As a consequence, formula $C\land \neg P$ is a shorthand for the set of
