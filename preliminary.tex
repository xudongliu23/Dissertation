...
and \tit{F} values.
We now defines what it means for a truth assignment to \tit{satisfies} 
a formula.
Let $v$ be a truth assignment, $X$ a variable, and $\varphi$ and $\psi$
propositional formulas.
We say $v$ satisfies $X$, denoted by $v \models X$, if $v$ has value \tit{T} on $X$.
Then, we have $v \models \neg \varphi$
if  $v \models \varphi$ does not hold, $v \models \varphi \land \psi$
...
if $v \models \varphi$ does not hold or $v \models \psi$ holds.
