...
We consider the domain of cars.
...
the preferences on \tit{Price} (\tit{Security}) depend upon the assignment made
...
because of the flipping sequence, the colored path in \figref{CPN_pref}.
Outcome $cps$ is optimal because not other outcome is
better.
...
of outcomes in consent with the preference graph \figref{CPN_pref}.
There are de facto two such total orders as follows.
		...
		\centering
		  \begin{tikzpicture}[->,>=stealth',node distance=1cm,main node/.style={circle,draw,font=\small}]
		    \node[rectangle,inner sep=0pt] (1)                                         {$\bar{c}p\bar{s}$};
				\node[rectangle,inner sep=0pt] (2) [below of=1]                            {\bluetext{$\bar{c}ps$}};
				\node[rectangle,inner sep=0pt] (3) [below of=2]                            {\bluetext{$\bar{c}\bar{p}s$}};
				\node[rectangle,inner sep=0pt] (4) [left of =3,xshift=-1cm,yshift=-0.5cm]  {\bluetext{$\bar{c}\bar{p}\bar{s}$}};
				\node[rectangle,inner sep=0pt] (5) [below of=3]                            {$c\bar{p}s$};
				\node[rectangle,inner sep=0pt] (6) [left of=5,xshift=-1cm,yshift=-0.5cm]   {\bluetext{$c\bar{p}\bar{s}$}};
...
have shown that the CPN-CONSISTENCY problem is PSPACE-complete.

For the CPN-DOMINANCE problem, its complexity depends.
...
is a tree-like preference formalism that has arisen from lexicographic
orderings \cite{Kaci:Pref} and lexicographic preferences \cite{10.2307/2296854}.
My research mostly deals with lexicographic preference trees
and other preference systems extending lexicographic preference trees,
such as preference trees and partial lexicographic preference trees.
...
over the car domain of binary \tit{Capacity}, \tit{Price}, and \tit{Security},
...
\noindent{\textbf{The Model.}}
An LP-tree consisting of $p$ binary attributes corresponds to a strict total order over
...
\noindent{\textbf{Problems and Complexity.}}
As any LP-tree induces a total order, the $\LP$-L-CONSISTENCE problem is trivial.
Moreover, an optimal alternative always exists and the $\LP$-OPTIMALITY-\rom{1} problem
is trivial too.  Similarly, the $\LP$-OPTIMALITY-\rom{2} and
$\LP$-OPTIMALITY-\rom{3} problems are easy to solve.
However, once labels in an LP-tree become propositional formulas,
the $\LP$-OPTIMALITY-\rom{2} and $\LP$-OPTIMALITY-\rom{3} problems are
no longer easy \cite{conf/adt15/liuT}.
These are our results and we will discuss it in \chref{PTrees}.

An LP-tree orders pairs of alternatives ($o_1,o_2$) by traversing down the tree
...
%
%	\begin{algorithm}[ht]
%	\KwIn{an LP-tree $T$, two alternatives $o_1$ and $o_2$ ($o_1 \not = o_2$)}
...
Unlike CP-nets and LP-tree that respectively express partial orders and total orders,
...
The \textit{tolerance degree} of outcome $o$ with regard to preference 
...
\[
 \TD_{(\phi,a)}(o) =
  ...
  \end{cases}
\]
Based on that, the tolerance degree of outcome $o$ with regard to a \emph{set}
