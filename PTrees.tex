...
impractical even for combinatorial domains with as few as 7 or 8 issues. 
However, 
...
Given an outcome $M \in \CD(\cI)$, we define the \emph{leaf of $M$ in $T$}
...
and $M'=x_1x_2x_3\neg x_4$,
we walk down the tree and find that $l_T(M)=l_1$ and $l_T(M')=l_2$. 
Thus, we have $M \succ_T M'$ since $l_1$ precedes $l_2$.
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.43\textwidth}
	  ...
	  \begin{tikzpicture}[->,>=stealth',node distance=1.2cm]
	    ...
	    \node[main node] (1) {$x_1$};
	    \node[rectangle,draw,inner sep=1pt] at (1,0) {$\neg x_1\!\! >\!\! x_1$};
	
	    \node[main node] (2) [below of=1] {$x_3$};
	    \node[rectangle,draw,inner sep=1pt] at (1,-1.2) {$x_3 \!\!>\!\! \neg x_3$};
	    
	    \node[main node] (3) [below left of=2] {$x_2$};
	    \node[rectangle,draw,inner sep=1pt] at (-1.85,-2.05) {$\neg x_2 \!\!>\!\! x_2$};
	    
	    \node[main node] (4) [below of=3] {$x_4$};
	    \node[rectangle split, rectangle split parts=2, draw,inner sep=1pt,font=\sffamily\small] at (-2.2,-3.3)
	        {
	          $x_2\!:\!x_4 \!\!>\!\! \neg x_4$
	          \nodepart{second}
	          $\neg x_2\!:\!\neg x_4 \!\!>\!\! x_4$
	    ...
	    \node[rectangle,draw,inner sep=1pt] at (1.85,-3.3) {$x_2 \!\!>\!\! \neg x_2$};
...
\label{thm:exp_small_ratio}
	Let $L(n)=2^n!$ be the number of linear orders of outcomes over $n$ binary issues,
