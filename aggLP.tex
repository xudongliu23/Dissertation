...
Clearly, the cardinality of $\cX(\cI)$, which we denote by $m$ throughout 
...
Given an alternative $\bar{x}=x_1x_2\ldots x_p$, we find its 
...
the LP tree $T$ in \figref{LPTree_full}(a). We note that the 
...
A full representation of an LP tree requires as much space as an
explicit enumeration of the preference order. However, sometimes LP 
trees can be represented in a much more concise way. For instance, if 
for some node $t$, its two subtrees are identical (that 
...
nodes. This subtree is now a single child of its parent and it is
neither left nor right, as it is associated with \emph{each} of the two
values of its parent node issue. To retain the preference information, 
at each node $t'$ of the subtree we place a \emph{conditional preference 
table}, and each preference in it specifies the preferred value for the 
issue labeling $t'$ given the value of the issue labeling $t$. In the
extreme case when for every node its two subtrees are identical, the 
tree can be collapsed to a path. 

We now formally extend the definition of an LP tree to cover the case 
with collapsed subtrees. From now on, an \emph{LP tree} is a tree in 
which every node except for the leaves has either two children, the 
left child and the right child, or a single ``straight down'' child, 
representing the collapse of the subtrees. The nodes are labeled 
with issues and, as before, we assume that each issue appears exactly 
once on each path from the root to a leaf. 

Further, each node in an LP tree is assigned a \emph{conditional 
...
The LP tree $T$ in \figref{LPTree_full} can be represented more
concisely as a (collapsed) CI-CP tree $v$ in \figref{LPTree}. Nodes
at depth one have their subtrees collapsed. In the tree in 
\figref{LPTree_full}, the subtrees of the node at depth 1 labeled 
\textit{Tran} are not only identical but also have the same preference 
information at every node. Thus, collapsing them does not incur any
growth in
...
\begin{figure}
  ...
  \begin{tikzpicture}[->,>=stealth',node distance=2cm,main node/.style={circle,draw,font=\small}]
    ...
    \node[rectangle,draw] at (-2.5,-1.4) {$c > m$};
    
    \node[main node] (3) [below of=2] {Tran};
    \node[rectangle split, rectangle split parts=2, draw, font=\sffamily\small] at (-2.8,-3.4)
        {
          $c:d>f$
          \nodepart{second}
          $m:f>d$
    ...
    \node[rectangle,draw] at (2.5,-1.4) {$f > d$};
    
    \node[main node] (5) [below of=4] {Dest};
    \node[rectangle,draw] at (2.5,-3.4) {$m > c$};
  ...
  \caption{An CI-CP LP tree $v$}
  \label{fig:LPTree}
\end{figure}
      
Finally, we recall the concept of a profile and of a positional
...
\begin{figure*}[!ht]
  ...
  \begin{subfigure}[b]{0.3\textwidth}
  	\centering
    \begin{tikzpicture}[->,>=stealth',node distance=0.8cm,main node/.style={circle,draw,font=\small}]
      \node[main node,inner sep=1pt] (1) {$X_2$};
      \node[rectangle,draw] at (1,0) {$1_2 > 0_2$};

      \node[main node,inner sep=1pt] (2) [below of=1] {$X_4$};
      \node[rectangle,draw] at (1,-0.8) {$0_4 > 1_4$};

      \node[main node,inner sep=1pt] (3) [below of=2] {$X_1$};
      \node[rectangle,draw] at (1,-1.6) {$1_1 > 0_1$};

      \node[rectangle,inner sep=1pt] (4) [below of=3] {$\vdots$};
      \node[rectangle,draw] at (1,-2.4) {$1 > 0$};

      \node[main node,inner sep=1pt] (5) [below of=4] {$X_p$};
      \node[rectangle,draw] at (1,-3.2) {$1_p > 0_p$};

      \node[main node,inner sep=1pt] (6) [below of=5] {$X_q$};
      \node[rectangle,draw] at (1,-4) {$1_q > 0_q$};
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}[->,>=stealth',node distance=0.8cm,main node/.style={circle,draw,font=\small}]
      ...
      \node[main node,inner sep=1pt] (3) [below of=2] {$X_1$};
      \node[rectangle,draw] at (1,-1.6) {$0_1 > 1_1$};

      \node[rectangle,inner sep=1pt] (4) [below of=3] {$\vdots$};
      \node[rectangle,draw] at (1,-2.4) {$0 > 1$};

      \node[main node,inner sep=1pt] (5) [below of=4] {$X_p$};
      \node[rectangle,draw] at (1,-3.2) {$0_p > 1_p$};
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}[->,>=stealth',node distance=0.8cm,main node/.style={circle,draw,font=\small}]
      \node[main node,inner sep=1pt] (1) {$X_2$};
      \node[rectangle,draw] at (1,0) {$0_2 > 1_2$};

      \node[main node,inner sep=1pt] (2) [below of=1] {$X_4$};
      \node[rectangle,draw] at (1,-0.8) {$1_4 > 0_4$};
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}[->,>=stealth',node distance=0.8cm,main node/.style={circle,draw,font=\small}]
      ...
      \node[rectangle,draw] at (1,0) {$0_4 > 1_4$};

      \node[main node,inner sep=1pt] (2) [below of=1] {$X_2$};
      \node[rectangle,draw] at (1,-0.8) {$1_2 > 0_2$};
...
\figref{proof3}. (In all three figures we only indicate the top two issues 
...
\subsection{Computational Complexity Results}
Now we summerize known computational complexity results of the evaluation problem
for $k$-approval \figref{kApp_comp}, $b$-Borda \figref{bBorda_comp} and 
$(k,l)$-approval \figref{klApp_comp} rules.
The results for $2^{p-1}$-approval, as well as those in \figref{kApp_comp_b}
and \figref{bBorda_comp_a} are attributed to Lang et al. \cite{lang:aggLP}.
\begin{figure}
	\centering
...
%%
%%The algorithm starts by randomly selecting issues to label the nodes on 
...
%%  \end{tikzpicture}
%%  
%%  \caption{A CI-CP tree of 4 issues}
...
%%Then we fix the number of votes (500) and vary the number of issues up to 20, again for 
...
%%
%%\subsubsection{Varying the number of issues and the number of votes}
%%%$p$ and $n$}
%%Our experiments on the winner problem for the three voting rules with 
%%the fixed number of issues are consistent with the property that the problem
%%is solvable in polynomial time. 
%%Both \emph{clingo} and \emph{clingcon} scale up well.
%%\figref{comparison:1} depicts the results for the cases with 
%%10 issues. When we fix the number of votes and vary the number of 
%%issues the time grows exponentially with $p$ (cf. \figref{comparison:2}),
...
%%rule to deal with, especially when the number of issues is large.
%%
...
%%\begin{figure*}[!ht]
%%	\centering
%%	\subfigure[$2^{p-2}$-Approval (1000votes/13issues)]{\includegraphics[width=0.45\textwidth]{figs/expAppMSCICP_1000v_13i.pdf}}%
%%	\subfigure[$2K$-Approval (1000votes/13issues)]{\includegraphics[width=0.45\textwidth]{figs/2kAppMSCICP_1000v_13i.pdf}}\\
%%	\subfigure[Borda (1000votes/4issues)]{\includegraphics[width=0.45\textwidth]{figs/bordaMSCICP_1000v_4i.pdf}}
%%
